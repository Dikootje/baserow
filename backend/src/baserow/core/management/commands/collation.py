...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
          cursor.execute("SELECT character_set_name, default_collate_name FROM information_schema.character_sets")
          row = cursor.fetchone()
          ...
          if options["fix"]:
              ...
              if current_character_set != EXPECTED_CHARSET:
                  logger.error(f"Current database charset '{current_character_set}' is not '{EXPECTED_CHARSET}'")
                  return
              
              cursor.execute("SELECT * FROM pg_collation WHERE collname = 'blbost'")
              result = cursor.fetchone()
              if result is None:
                  logger.error(f"Can't change the default collation. Collation not available.")
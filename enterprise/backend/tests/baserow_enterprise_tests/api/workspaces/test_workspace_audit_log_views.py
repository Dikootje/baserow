...
def test_admins_can_not_access_workspace_audit_log_endpoints_without_an_enterprise_license(
    ...
    response = api_client.get(
        reverse(
            f"api:enterprise:workspace:audit_log:{url_name}",
...
@pytest.mark.parametrize("url_name", ["users", "action_types", "list"])
@override_settings(DEBUG=True)
def test_non_admins_can_not_access_workspace_audit_log_endpoints(
    api_client, enterprise_data_fixture, url_name
):
...
@pytest.mark.django_db
@override_settings(DEBUG=True)
def test_admins_can_not_export_workspace_audit_log_to_csv_without_an_enterprise_license(
    ...
    response = api_client.post(
        reverse(
            "api:enterprise:workspace:audit_log:export",
            kwargs={"workspace_id": workspace.id},
        ),
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
...
@pytest.mark.django_db
@override_settings(DEBUG=True)
def test_non_admins_can_not_export_workspace_audit_log_to_csv(
    api_client, enterprise_data_fixture
):
    enterprise_data_fixture.enable_enterprise()
...
def test_workspace_audit_log_user_filter_returns_only_workspace_users(
    ...
    response = api_client.get(
        reverse(
            "api:enterprise:workspace:audit_log:users",
    ...
    response = api_client.get(
        reverse(
            "api:enterprise:workspace:audit_log:users",
            kwargs={"workspace_id": workspace_2.id},
        ),
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
